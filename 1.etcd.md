...
```bash
{
  ...
  for master in "${MASTERS[@]}"; do
    echo ">> Copying etcd client certs to $master"

    sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no root@"$master" \
    ...
    sshpass -p "$PASSWORD" scp -o StrictHostKeyChecking=no \
      ...
      root@"$master":/etc/kubernetes/pki/etcd/

    sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no root@"$master" "
      chmod 644 /etc/kubernetes/pki/etcd/ca.pem &&
      chmod 644 /etc/kubernetes/pki/etcd/apiserver-etcd-client.pem &&
      chmod 600 /etc/kubernetes/pki/etcd/apiserver-etcd-client-key.pem
    "
  done
}
